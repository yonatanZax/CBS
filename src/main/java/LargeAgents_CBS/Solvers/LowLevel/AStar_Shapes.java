...
public class AStar_Shapes extends SingleAgentAStar_Solver {
    ...
    private List<Move> getAllCellsMoves(Move move){
        GraphLocationGroup currentLocation = (GraphLocationGroup) move.currLocation;
        GraphLocationGroup prevLocation = (GraphLocationGroup) move.prevLocation;
        List<GraphMapVertex_LargeAgents> groupCurrentLocation =  new ArrayList<>(){{addAll(currentLocation.getAllCells());}};
        List<GraphMapVertex_LargeAgents> groupPrevLocation =  new ArrayList<>(){{addAll(prevLocation.getAllCells());}};

        List<Move> moves = new ArrayList<>();

        for (int i = 0; i < groupCurrentLocation.size(); i++) {
            moves.add(new Move(move.agent, move.timeNow, groupPrevLocation.get(i),groupCurrentLocation.get(i)));
        }
