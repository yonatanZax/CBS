...
public class AStar_LargeAgents extends SingleAgentAStar_Solver {
    ...
    protected boolean initOpen() {
        ...
        if(existingPlan.size() > 0){
            ...
            GraphMapVertex_LargeAgents referencePoint_cur = ((GraphLocationGroup)lastExistingMove.currLocation).getReferencePoint();
            GraphMapVertex_LargeAgents referencePoint_prev = ((GraphLocationGroup)lastExistingMove.prevLocation).getReferencePoint();
            GraphLocationGroup refPointGroup_cur = new GraphLocationGroup(new GraphMapVertex_LargeAgents[][]{{referencePoint_cur}});
            GraphLocationGroup refPointGroup_prev = new GraphLocationGroup(new GraphMapVertex_LargeAgents[][]{{referencePoint_prev}});
            ...
            for (I_Location destination: neighborCellsIncludingCurrent) {
                ...
                GraphMapVertex_LargeAgents referencePoint_cur = ((GraphLocationGroup)possibleMove.currLocation).getReferencePoint();
                GraphMapVertex_LargeAgents referencePoint_prev = ((GraphLocationGroup)possibleMove.prevLocation).getReferencePoint();
                GraphLocationGroup refPointGroup_cur = new GraphLocationGroup(new GraphMapVertex_LargeAgents[][]{{referencePoint_cur}});
                GraphLocationGroup refPointGroup_prev = new GraphLocationGroup(new GraphMapVertex_LargeAgents[][]{{referencePoint_prev}});
                Move refPointMove = new Move(possibleMove.agent, possibleMove.timeNow, refPointGroup_prev, refPointGroup_cur);
